import {GetAllProducts, GetAllRetailerIds, filter_all_lineage, filter_strain_type_lineage, getAllProducts} from '../utils/fetch_querys.js';
...
window.addEventListener('DOMContentLoaded',  async () => {
    ...
    setTimeout( async () => {
        ...
        products.map( items => {
                        ...
                        container_image.href=`/views/product-details.html?id=${items.id}`;
                        let image = createElementHtml('img');
                        image.className='card-img-top';
                        image.id='imagen-product';
                        image.src=`${items.image !== null ? items.image : '../assets/images/errors-images/image-not-found.jpeg'}`;
                        image.alt=`${items.name}`;

                        let link_a_item_brand = createElementHtml('a');
                        link_a_item_brand.href=`/views/product-details.html?id=${items.id}`;
                        ...
                        etiqueta_a_link_item_name.href=`/views/product-details.html?id=${items.id}`;
                        let etiqueta_h6_item_name = createElementHtml('h6');
                        etiqueta_h6_item_name.className='product-name mb-2 itemname';
                        etiqueta_h6_item_name.textContent=`${items.name}`;
                        ...
                        span_text_price.textContent=`${items.variants[0].priceMed}`;
                        let span_text_weights = createElementHtml('span');
                        span_text_weights.className='er-each jceachformat';
                        span_text_weights.textContent=`/${items.variants[0].option}`;
                        ...
                        btn_product_details.href=`/views/product-details.html?id=${items.id}`;
